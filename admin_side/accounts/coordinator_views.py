...
def mark_attendance(request):
    ...
    # If coordinator did not explicitly select a program, auto-select their first assigned program
    if not selected_program:
        if courses.exists():
            first_course = courses.first()
            selected_program = f"course_{first_course.id}"
        elif internships.exists():
            first_internship = internships.first()
            selected_program = f"internship_{first_internship.id}"
...
def approve_certificate(request, certificate_id):
    ...
    if action == 'approve':
        # Check attendance eligibility before approving
        attendance_percentage = Attendance.get_attendance_percentage(application)
        if attendance_percentage < 75:
            messages.error(request, f'Student not eligible for a certificate (attendance {attendance_percentage}%, required 75%).')
            return redirect('coordinator:certificates')

        certificate.status = 'APPROVED'
        certificate.approved_by = coordinator
        certificate.is_eligible = True
